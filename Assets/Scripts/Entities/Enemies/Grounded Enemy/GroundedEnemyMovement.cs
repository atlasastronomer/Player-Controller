...
public class GroundedEnemyMovement : MonoBehaviour {
    ...
    private bool _enemyJumped = false;
    ...
    void Update() {
        ...
        if ((_isWallSlidingRight || _isWallSlidingLeft)) {
            _displacement.y = Mathf.SmoothDamp(_displacement.y, _wallSlideDisplacementY, ref _displacementYSmoothing,  _wallSlideDeceleration);
        }
        else {
        ...
        if (_isDashing) {
            ...
            _displacement.x = Mathf.SmoothDamp(_displacement.x, targetdisplacementX, ref _displacementXSmoothing, (_isGrounded) ? accelerationTimeGrounded : accelerationTimeAirborne);
        }
