...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update() 
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space)) 
        {
            ...
            {
                // Wall Climb
                if (Mathf.Sign(Input.GetAxisRaw("Horizontal")) == Mathf.Sign(_facingDirection) && Input.GetAxisRaw("Horizontal") != 0) 
                {
                    WallClimb();
                }
                // Wall Hop
        ...
        if (_isDashing) 
        {
            displacement.x = Mathf.SmoothDamp(displacement.x, dashSpeed * _facingDirection, ref _displacementXSmoothing, dashAccelerationTime);
            displacement.y = 0f;
        }
        else 
        {
