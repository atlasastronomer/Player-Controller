...
namespace Core.Movement
{
    ...
    public class Controller2D : RaycastController
    {
        ...
        private void HorizontalCollisions(ref Vector3 displacement)
        {
            ...
            {
                Vector2 rayOrigin = (Mathf.Approximately(directionX, -1))
                ...
                if (hit)
                {
                    ...
                    if (!Collisions.ClimbingSlope || slopeAngle > _maxClimbAngle)
                    {
                        ...
                        {
                            displacement.y = Mathf.Tan(Collisions.SlopeAngle * Mathf.Deg2Rad) *
        ...
        private void VerticalCollisions(ref Vector3 displacement)
        {
            ...
            {
                Vector2 rayOrigin = (Mathf.Approximately(directionY, -1))
                    ? rayCastOrigins.BottomLeft
            ...
            if (Collisions.ClimbingSlope)
            {
                ...
                Vector2 rayOrigin =
                    ((Mathf.Approximately(directionX, -1)) ? rayCastOrigins.BottomLeft : rayCastOrigins.BottomRight) +
                    Vector2.up * displacement.y;
                RaycastHit2D hit = Physics2D.Raycast(rayOrigin, Vector2.right * directionX, rayLength, collisionMask);
